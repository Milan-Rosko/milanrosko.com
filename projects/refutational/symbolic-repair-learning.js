/*
  js/symbolic-repair-learning.js
  By Milan Rosko (c)
...
(function () {
  ...
  function symbolicRepairFirstWitness(state, table, gamma, delta) {
  ...
  function generateTypedTrace(parsed, featNames, table, gamma, delta, maxCycles) {
    ...
    while (state.updates < maxCycles) {
      ...
      const res = symbolicRepairFirstWitness(state, table, gamma, delta);
      if (!res.changed) {
        state.epoch += 1;
        snapshots.push({ ...state, W: clone2D(state.W), S: clone2D(state.S) });
  ...
  function generateGDTrace(parsed, featNames, table, lr, maxCycles, seed, scanOrder) {
    ...
    while (updates < maxCycles) {
      ...
      const ev = evalGD(table, K, W);
      const pred = stableArgmaxStar(ev.logits[i]);
      const note = `GD @i=${i} (y=${yi}, pred=${pred}): loss≈${ev.loss.toFixed(4)}, acc=${(ev.acc * 100).toFixed(1)}%`;
